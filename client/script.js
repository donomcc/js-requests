...
/*
    Create a function that changes sayHelloButton's background color to black and its
     text color to white (you can use the .style object or create a CSS class 
...
sayHelloButton.addEventListener("mouseover", changeColorBlack);

//  Question:  How come when I make the function changeColorBlack() it automatically runs?
...
sayHelloButton.addEventListener("mouseout", resetColors);
...
const repeatMyParam = () => {
  axios.get(`http://localhost:3000/repeat/cheese`).then((res) => {
...
const queryTest = () => {
  axios
    .get("http://localhost:3000/query-test/?name=jack&id=123")
    .then((res) => {
      console.log(res);
    });
};
...
/*
    ...
    field and a button. The input field should have a placeholder that tells the user to enter a food. And the button should indicate that it will add food into a list. 
    ...
    Next, create an object called body inside the function. It should have one key-value pair. The key should be newFood (make sure to match the case and spelling exactly) and the 
